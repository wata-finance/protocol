...
/**
 * Staking mLTC_USDT mdex LP token for HLTC + HDOGE + HPT reward.
 */
contract LpStaking is VersionedInitializable, ReentrancyGuard {
  ...
  address public mLTCUSDTLpToken;
  address public mLTC;
  ...
  function stake(uint256 amount) external nonReentrant refreshAccountReward(msg.sender) {
    require(amount > 0, 'Cannot stake 0');

    lpBalances[msg.sender] = lpBalances[msg.sender].add(amount);
    totalSupply = totalSupply.add(amount);
    IERC20(mLTCUSDTLpToken).safeTransferFrom(msg.sender, address(this), amount);
    emit Staked(msg.sender, amount);
  }

  function withdraw(uint256 amount) public nonReentrant refreshAccountReward(msg.sender) {
    require(amount > 0, 'Cannot withdraw 0');

    totalSupply = totalSupply.sub(amount);
    lpBalances[msg.sender] = lpBalances[msg.sender].sub(amount);
    IERC20(mLTCUSDTLpToken).safeTransfer(msg.sender, amount);
    emit Withdrawn(msg.sender, amount);
  }

  function exit() external {
    withdraw(lpBalances[msg.sender]);
    claimReward();
  }
  ...
  function calRewardPerToken(uint256 _curRewardAmount) internal view returns (uint256) {
    ...
    if (startTime < endTime && startTime >= curPeriodStart && endTime <= curPeriodEnd) {
      ...
      uint256 stakingSupply = IStaking(mLTCStaking).totalSupply();
      uint256 curRewardAmount =
        _curRewardAmount.mul(mLTCTotalSupply.sub(stakingSupply)).div(mLTCTotalSupply);
  ...
  function getUnclaimedHLTC(address account) public view returns (uint256) {
    ...
    return lpBalances[account].mul(rewardPerToken).div(1e18).add(userUnclaimedHLTC[account]);
  }
  ...
  function getUnclaimedHDOGE(address account) public view returns (uint256) {
    ...
    return lpBalances[account].mul(rewardPerToken).div(1e18).add(userUnclaimedHDOGE[account]);
  }
  ...
  function getUnclaimedHPT(address account) public view returns (uint256) {
    ...
    return lpBalances[account].mul(rewardPerToken).div(1e18).add(userUnclaimedHPT[account]);
  }
